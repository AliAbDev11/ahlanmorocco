...
import { generateReport, downloadRecentReport } from '@/lib/reportGenerator';
...
const reportTemplates: ReportTemplate[] = [
  { id: 'daily-operations', name: 'Daily Operations Report', description: 'Overview of daily hotel operations, occupancy, and key metrics', icon: TrendingUp, color: 'text-blue-500', frequency: 'Daily' },
  { id: 'weekly-occupancy', name: 'Weekly Occupancy Report', description: 'Detailed occupancy analysis with trends and forecasts', icon: DoorOpen, color: 'text-green-500', frequency: 'Weekly' },
  { id: 'monthly-revenue', name: 'Monthly Revenue Report', description: 'Comprehensive revenue breakdown by category and source', icon: ShoppingCart, color: 'text-accent', frequency: 'Monthly' },
  { id: 'guest-satisfaction', name: 'Guest Satisfaction Report', description: 'Guest feedback, complaints, and satisfaction metrics', icon: Users, color: 'text-purple-500', frequency: 'Monthly' },
  { id: 'service-performance', name: 'Service Performance Report', description: 'Service request response times and resolution rates', icon: Wrench, color: 'text-orange-500', frequency: 'Weekly' },
  { id: 'complaint-analysis', name: 'Complaint Analysis Report', description: 'Detailed analysis of complaints, trends, and resolution', icon: MessageSquareWarning, color: 'text-destructive', frequency: 'Monthly' },
];
...
const ManagerReports = () => {
  ...
  const handleGenerateReport = async (template: ReportTemplate) => {
    ...
    try {
      ...
      toast.success(`${template.name} generated successfully`, {
        description: `Format: ${exportFormat.toUpperCase()} | Range: ${format(dateRange.from, 'MMM dd')} – ${format(dateRange.to, 'MMM dd, yyyy')}`,
      });
    } catch {
  ...
  const handleDownloadRecent = (report: typeof recentReports[0]) => {
    downloadRecentReport(report.name, report.format);
    toast.success(`Downloading ${report.name}`, { description: `${report.format} • ${report.size}` });
  };
  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-muted-foreground mt-1">Generate and export custom reports for hotel operations and analytics.</p>
      </div>
            ...
            {recentReports.map((report, index) => (
              <div key={index} className="flex items-center justify-between p-3 rounded-lg border border-border hover:bg-muted/50 transition-colors">
                ...
                <div className="flex items-center gap-2">
                  ...
                  <Button variant="ghost" size="sm" onClick={() => handleDownloadRecent(report)}>
